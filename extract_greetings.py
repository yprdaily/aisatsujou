...
TITLE_KEYWORDS = [
    ...
    "担当者", "代表",
    # 役職・肩書系
    "支配人", "局長", "事務局長", "受任者", "事務受任者",
    # 資格・士業
    "行政書士", "司法書士", "税理士", "弁護士", "公認会計士", "社会保険労務士",
    # 採用系慣用表現
    "採用担当", "採用ご担当", "採用御担当",
]
...
DEPT_SUFFIXES = (
    "部", "課", "室", "センター", "ｾﾝﾀｰ", "本部", "支店", "営業所", "事業部", "事務所", "店",
    "グループ", "ｸﾞﾙｰﾌﾟ", "チーム", "ﾁｰﾑ", "事務局",
)
...
def extract_last_fullname(text: Any, surname_whitelist: set[str]) -> str | None:
    ...
    # 姓スロットに括弧が含まれる場合は会社名・事務所名等が混入している可能性が高い
    if contains_parentheses_any_width(last_family):
        return None
